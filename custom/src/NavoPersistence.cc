...
void NavoPersistence::saveLakes(){QSettings s;s.setValue("navo/lakes",jsonList(_lakes));s.sync();}
QString NavoPersistence::saveLake(const QVariantMap& input){QVariantMap lake=input;QString id=lake.value("id").toString();if(id.isEmpty())id=QString("lake-%1").arg(QDateTime::currentMSecsSinceEpoch());lake["id"]=id;if(!lake.contains("createdAt"))lake["createdAt"]=QDateTime::currentDateTimeUtc().toString(Qt::ISODate);lake["updatedAt"]=QDateTime::currentDateTimeUtc().toString(Qt::ISODate);for(int i=0;i<_lakes.size();++i)if(_lakes[i].toMap().value("id").toString()==id){auto old=_lakes[i].toMap();for(auto it=lake.cbegin();it!=lake.cend();++it)old[it.key()]=it.value();_lakes[i]=old;saveLakes();emit lakesChanged();return id;}_lakes.prepend(lake);saveLakes();emit lakesChanged();return id;}
...
bool NavoPersistence::saveLakeState(const QString& lakeId,const QVariantMap& state){for(int i=0;i<_lakes.size();++i){auto m=_lakes[i].toMap();if(m.value("id").toString()!=lakeId)continue;m["state"]=state;m["updatedAt"]=QDateTime::currentDateTimeUtc().toString(Qt::ISODate);_lakes[i]=m;saveLakes();emit lakesChanged();return true;}return false;}
