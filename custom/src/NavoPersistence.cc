...
bool NavoPersistence::saveLakeState(const QString& lakeId,const QVariantMap& state){
 ...
 for(int i=0;i<_lakes.size();++i){auto m=_lakes[i].toMap();if(m.value("id").toString()!=lakeId)continue;m["state"]=state;m["updatedAt"]=QDateTime::currentDateTimeUtc().toString(Qt::ISODate);_lakes[i]=m;saveLakes();emit lakesChanged();return true;}
 ...
 _lakes.prepend(lake);saveLakes();emit lakesChanged();return true;
}
