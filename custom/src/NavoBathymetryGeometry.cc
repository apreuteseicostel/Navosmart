...
void NavoBathymetryGeometry::rebuild(){
 ...
 const int stride=9*sizeof(float);QByteArray vb(_vertices.size()*stride,Qt::Uninitialized);float*out=reinterpret_cast<float*>(vb.data());QVector<QVector3D>pos;pos.reserve(_vertices.size());
