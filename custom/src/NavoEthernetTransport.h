...
class NavoEthernetTransport : public QObject
{
    ...
    Q_PROPERTY(bool dataAlive READ dataAlive NOTIFY dataAliveChanged)
    Q_PROPERTY(bool autoReconnect READ autoReconnect WRITE setAutoReconnect NOTIFY autoReconnectChanged)
    Q_PROPERTY(QString status READ status NOTIFY statusChanged)

public:
    explicit NavoEthernetTransport(QObject* p = nullptr);

    QString host() const { return _host; }
    quint16 port() const { return _port; }
    bool udp() const { return _udp; }
    bool connected() const { return _connected; }
    bool dataAlive() const { return _dataAlive; }
    ...
    Q_INVOKABLE void connectEndpoint();
    Q_INVOKABLE void disconnectEndpoint();

signals:
    void endpointChanged();
    void connectedChanged();
    void dataAliveChanged();
